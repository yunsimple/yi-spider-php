...
class RedisController
{
    ...
    public static function setString($key, $value, $expire = false)
    {
        if(is_int($key) || is_string($key)){
     ...
     */
    public static function stringReturn($key, $value = 1, $expire = 3600){
...
/*            foreach ($arr as &$value) {
     ...
     * @param $key
     * @param int $value 自增值
     * @param int $expire -1时为不过期
     * @return int|mixed
     */
    public static function incrEx($key, $value = 1, $expire = 3600){
        if (!self::$redis->exists($key)){
